          ...
          {launchFacts ? (
            <section className='rounded-2xl border border-green-500/30 bg-green-500/10 p-6'>
              ...
              <p className='mt-3 text-sm text-green-100'>
                The founder vault holds {FOUNDER_ALLOCATION_WHOLE.toLocaleString('en-US')} wMATRIX against exactly
                the same amount of MATRIX in native escrow. New locks require at least{' '}
                {(MinBridgeLockAmount / NativeUnit).toLocaleString('en-US')} MATRIX and the deployed{' '}
                {ATTESTOR_THRESHOLD}-of-{ATTESTOR_THRESHOLD} attestor threshold.
              </p>
              <div className='mt-4 space-y-1 text-xs text-green-100'>
                <p className='break-all'>Founder vesting vault: <a className='font-mono underline' href={`${addressUrl(launchFacts.explorerUrl, launchFacts.vaultAddress)}#code`} target='_blank' rel='noopener noreferrer'>{launchFacts.vaultAddress}</a></p>
                <p className='break-all'>Founder mint: <a className='font-mono underline' href={txUrl(launchFacts.explorerUrl, launchFacts.mintTx)} target='_blank' rel='noopener noreferrer'>{launchFacts.mintTx}</a></p>
                <p><a className='underline' href={launchFacts.evidenceUrl} target='_blank' rel='noopener noreferrer'>Read the launch evidence and operating policy</a></p>
              </div>
            </section>
          ) : null}
